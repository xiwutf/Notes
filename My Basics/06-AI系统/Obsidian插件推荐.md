...
常用查询语法：
  TABLE status, priority FROM "02-项目" WHERE type = "project"
  LIST FROM "03-知识库" WHERE status = "种子"
  TABLE length(rows) AS "数量" FROM "" GROUP BY type
```

**在本系统中的用法**

- [[00-工作台/首页|Dashboard Home]] — 项目列表、最近思考、系统统计
- [[02-项目/MOC-Projects|MOC-Projects]] — 按状态筛选项目
- [[03-知识库/MOC-Knowledge|MOC-Knowledge]] — 知识状态追踪
...
模板引擎——创建 Daily Note、Project、Thinking 笔记时自动填充日期、生成 Frontmatter、插入双向链接。本系统所有 [[07-模板|Templates]] 都使用 Templater 语法。
...
```
Settings → Templater:
  Template folder location: 07-模板
  ✅ Trigger Templater on new file creation
  ✅ Enable Folder Templates:
     08-日记/ → Daily Note Template
     02-项目/ → Project Template
     03-知识库/读书/ → Reading Template
     03-知识库/触动/ → Insight Template
     03-知识库/思考/ → Thinking Template
     03-知识库/ → Knowledge Template
     08-日记/复盘/ → Weekly Review Template
  User scripts folder: 07-模板/脚本
     new_book → 命令面板创建读书笔记
     new_insight → 命令面板创建 Insight
...
```markdown
2026-06-01        → 2026-05-26
2026-05-25                     → 当前文件名
[[08-日记/2026-05-31|昨日]]  → 昨日链接
```
...
```
Settings → Tasks:
  Global Filter: 留空（或设置 path includes 08-日记 OR 02-项目）
...
Settings → Daily Notes（Core Plugin）:
  ✅ Enable Daily Notes
  New file location: 08-日记
  Template file location: 07-模板/日记模板
...
→ [[00-工作台/首页|Dashboard]] | [[认知系统说明书|认知系统说明书]]
